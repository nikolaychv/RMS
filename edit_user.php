...
{
  $query = 'SELECT * FROM tbl_user WHERE user_id = '.$_GET['edit_id'].' LIMIT 1';
  $result = mysqli_query($connection, $query);
  
  if(!mysqli_num_rows($result))
  {
    header('location: users.php?error=Failed to change the selected user...');
    exit(); 
  }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_POST['user_username'] != null && 
      ...
      $the_user_status = $_POST['user_status'];
        ...
        $first_sql = 'UPDATE tbl_user SET user_username = "'.$the_username.'", user_password = "'.$the_password.'",
                                          user_full_name = "'.$the_user_full_name.'", user_email = "'.$the_user_email.'",
                                          user_type = "'.$the_user_type.'", user_status = "'.$the_user_status.'"
                                          WHERE user_id = '.$_GET['edit_id'].' LIMIT 1';
                       
        $first_result = mysqli_query($connection, $first_sql);
               
            if ($first_result) {
            header("location: users.php?success=Your account has been updated successfully.");
	    exit();
            }
            else {
	    header('location: users.php?error=Failed to change the selected user...');
...
<div class="edit-content">
  ...
  <input type="password" name="password_check" placeholder="Confirm the password..."><br>
        ...
        <a href="users.php"><img id="go-back-image" src="images/go-back-image.png" alt="Go back"></a>
     	<button type="submit" id="submit-button">Edit user</button>
               </div> 
     </form> 
